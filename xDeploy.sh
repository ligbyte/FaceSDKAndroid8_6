...
outputsdkaarpath=$outputpath"/aar"
...
function BDFaceConfig
{
    ...
    ANDROID_NDK_HOME=/home/scmtools/buildkit/android-ndk-r17c
    JAVA_GRADLE=/home/scmtools/buildkit/gradle/gradle-5.1.1/bin
...
#    mkdir $outputsdkdemopath
    ...
    rm -rf outputsdkaarpath
    mkdir $outputsdkaarpath
    # output最终输出产物路径
...
{
    echo 3.1 ------- copy assets -------
    sdkassetpath=$workpath"/facelibrary/src/main/assets/*"
    ls $workpath"/facelibrary/src/main/assets/"
    cp -rf $sdkassetpath  $outputsdkassetpath

    echo 3.2 ------- modify config.ini -------
    cd $outputsdkassetpath
    find . -type f | grep -v "DS_Store" >> ../config.ini

    echo   ---------------------------------------------------------------
    echo 4.1 ------- copy FaceSDK lib -------
#    facelibrarysopath=$workpath"/facelibrary/build/intermediates/merged_jni_libs/*"
...
#    facelibrarysecjarpath=$workpath"/app/libs/*"
#    facelibrarysopath=$workpath"/faceplatform-ui/build/intermediates/library_and_local_jars_jni/release/*"
    facelibraryjarspath=$workpath"/facelibrary/libs/*"
    ls $facelibraryjarspath
    cp -rf $facelibraryjarspath $outputsdklibspath
    ls $outputsdklibspath
    facelibraryaarpath=$workpath"/facelibrary/build/outputs/aar/*"
    ls $facelibraryaarpath


#    ls $workpath"/facelibrary/build/intermediates/bundles/merged_jni_libs/*"
#    cp -rf $facelibrarysopath $outputsdklibspath
    echo ====after copy==
    ls $outputsdklibspath"/armeabi-v7a/"
    cp -rf $facelibraryaarpath $outputsdkaarpath
    ls $outputsdkaarpath
