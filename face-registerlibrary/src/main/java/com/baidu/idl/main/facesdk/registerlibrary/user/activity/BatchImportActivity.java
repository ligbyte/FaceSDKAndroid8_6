...
//        ImportFileManager.getInstance().release();
    ...
    private void initData() {
        ...
        if (ShareManager.getInstance(mContext).getDBState()) {
            ...
            mButtonImport.setText(getResources().getString(R.string.re_import));
        }
        ...
        if (id == R.id.button_import) {   // 点击导入数据按钮
            if (!mImporting) {
                if (!FileUtils.isSdCardAvailable()) {
                    ToastUtils.toast(mContext, getResources().getString(R.string.import_sd_card_required));
                    return;
                }

                if (getResources().getString(R.string.re_import).equals(mButtonImport.getText())) {
                    mTipDialog.show();
                    mTipDialog.setTextTitle(getResources().getString(R.string.re_import));
                    mTipDialog.setTextMessage(getResources().getString(R.string.dialog_import_message));
                    mTipDialog.setTextConfirm(getResources().getString(R.string.dialog_import_confirm));
            ...
            mTipDialog.setTextMessage(getResources().getString(R.string.dialog_delete_message));
            mTipDialog.setTextConfirm(getResources().getString(R.string.dialog_delete_confirm));
            mTipDialog.setCancelable(false);
        }
    }
    ...
    public void onImporting(final int totalCount, final int successCount, final int failureCount,
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                mTextImportFinish.setText(getResources().getString(R.string.data_statistics) + totalCount);
                mTextImportSuccess.setText(getResources().getString(R.string.import_success) + successCount);
                mTextImportFailure.setText(getResources().getString(R.string.import_failure) + failureCount);
...
//                ImportFileManager.getInstance().release();
    ...
    public void onConfirm(String tipType) {
        ...
        // 根据对话框的内容来判断操作
        if (getResources().getString(R.string.re_import).equals(tipType)) {
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
        ...
        } else if (getResources().getString(R.string.dialog_delete_title).equals(tipType)) {
            // 清空用户表
...
//            ShareManager.getInstance(mContext).setDBState(false);
            ...
            mButtonImport.setText(getResources().getString(R.string.import_data));
            mImporting = false;
        }
    }
}
