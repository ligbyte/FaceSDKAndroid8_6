...
//                                 更新进度
    ...
    public int onQualityCheck(FaceInfo faceInfo) {
        ...
        if (faceInfo != null) {
            ...
            // 角度过滤
            if (Math.abs(faceInfo.yaw) > SingleBaseConfig.getBaseConfig().getGesture()) {
                return 4;
            } else if (Math.abs(faceInfo.roll) > SingleBaseConfig.getBaseConfig().getGesture()) {
                return 4;
            } else if (Math.abs(faceInfo.pitch) > SingleBaseConfig.getBaseConfig().getGesture()) {
                return 4;
            }
